...
export default function SavingsGoalCard({ goal }: { goal: SavingsGoal }) {
  ...
  return (
    <div className="flex flex-col items-center gap-2 p-3 rounded-xl" style={{ background: 'var(--card)' }}>
      ...
      <div className="text-center -mt-2">
        ...
        <p className="text-xs mt-0.5" style={{ color: 'var(--text-3)' }}>{fmt(goal.current)} / {fmt(goal.goal)}</p>
      </div>
    </div>
  )
}