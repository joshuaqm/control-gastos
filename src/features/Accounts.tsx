...
export default function AccountsScreen({
  ...
  const [accountsWithMovements, setAccountsWithMovements] =
  ...
  const load = async () => {
    setLoading(true)
    try {
      const [list, txns, installments] = await Promise.all([
        fetchAccounts(),
        fetchTransactions(),
        fetchInstallments(),
      ])
  ...
  const handleInterestAdjust = async (data: {
    amount: number
    month: string
  }) => {
              ...
              >
                ...
                >
                  ...
                  {a.interest_rate != null && (
                    <span>
                      Tasa: {a.interest_rate}%
                      {a.last_interest_at
                        ? ` · último ajuste ${new Date(a.last_interest_at).toLocaleDateString("es-MX")}`
                        : ""}
                    </span>
                  )}
                ...
                <div className="flex gap-2 mt-4">
                  ...
                  <button
                    ...
                    title={
                      accountsWithMovements.has(a.id)
                    ...
                    style={{
                      ...
                      color: accountsWithMovements.has(a.id)
                        ? "#4B5563"
                        : "#F87171",
                      border: "1px solid rgba(239,68,68,0.2)",
                      cursor: accountsWithMovements.has(a.id)
                        ? "not-allowed"
                        : "pointer",
                    }}
                  >
