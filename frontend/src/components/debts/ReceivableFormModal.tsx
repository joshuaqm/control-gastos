...
export default function ReceivableFormModal({
  ...
  const [originalAmount, setOriginalAmount] = useState(receivable ? receivable.original_amount.toString() : '')
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(8px)' }}>
      <div className="glass animate-slide-up rounded-2xl p-6 w-full max-w-md" style={{ border: '1px solid rgba(124,58,237,0.2)' }}>
        <div className="flex items-center justify-between mb-5">
          <h3 className="text-lg font-semibold">{receivable ? 'Editar por cobrar' : 'Nuevo por cobrar'}</h3>
          <button onClick={onClose} style={{ color: '#6B6B85' }}><X size={20} /></button>
        </div>

        <div className="flex flex-col gap-3">
          <div>
            <label className="block text-xs font-medium mb-1" style={{ color: '#A0A0B8' }}>Persona</label>
            <input
              value={person}
          ...
          <div>
            <label className="block text-xs font-medium mb-1" style={{ color: '#A0A0B8' }}>Concepto (opcional)</label>
            <input
              value={description}
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs font-medium mb-1" style={{ color: '#A0A0B8' }}>Fecha de pago</label>
              <input
                value={dueDate}
