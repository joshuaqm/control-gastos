...
export default function InstallmentFormModal({
  ...
  const [monthlyAmount, setMonthlyAmount] = useState(installment ? installment.monthly_amount.toString() : '')
  const [monthsTotal, setMonthsTotal] = useState(installment ? installment.months_total.toString() : '')
  const [monthsPaid, setMonthsPaid] = useState(installment ? installment.months_paid.toString() : '0')
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(8px)' }}>
      <div className="glass animate-slide-up rounded-2xl p-6 w-full max-w-md" style={{ border: '1px solid rgba(124,58,237,0.2)' }}>
        <div className="flex items-center justify-between mb-5">
          <h3 className="text-lg font-semibold">{installment ? 'Editar crédito a meses' : 'Nuevo crédito a meses'}</h3>
          <button onClick={onClose} style={{ color: '#6B6B85' }}><X size={20} /></button>
        </div>

        <div className="flex flex-col gap-3">
          <div>
            <label className="block text-xs font-medium mb-1" style={{ color: '#A0A0B8' }}>Descripción</label>
            <input
              value={description}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium mb-1" style={{ color: '#A0A0B8' }}>Mensualidad</label>
              <input
                value={monthlyAmount}
            ...
            <div>
              <label className="block text-xs font-medium mb-1" style={{ color: '#A0A0B8' }}>Meses totales</label>
              <input
                value={monthsTotal}
          ...
          {installment && (
            <div>
              <label className="block text-xs font-medium mb-1" style={{ color: '#A0A0B8' }}>Meses pagados</label>
              <input
                value={monthsPaid}
          ...
          <p className="text-xs px-3 py-2 rounded-lg" style={{ background: 'rgba(124,58,237,0.1)', border: '1px solid rgba(124,58,237,0.25)', color: '#C4B5FD' }}>
            Total: {totalAmount.toLocaleString('es-MX', { style: 'currency', currency: 'MXN' })} · Saldo ocupado: {monthlyAmount && monthsTotal ? (Number(monthlyAmount) * (Number(monthsTotal) - Number(monthsPaid))).toLocaleString('es-MX', { style: 'currency', currency: 'MXN' }) : '—'}
          </p>
